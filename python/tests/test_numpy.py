...
class Test(unittest.TestCase):
    ...
    def test_numpy1d(self):
        before = numpy.histogram(
            numpy.random.normal(0, 1, int(1e6)), bins=100, range=(-5, 5)
        )
        after = aghast.to_numpy(aghast.from_numpy(before))
        assert numpy.array_equal(before[0], after[0])
        assert numpy.array_equal(before[1], after[1])

    def test_numpy2d(self):
        before = numpy.histogram2d(
            x=numpy.random.normal(0, 1, int(1e6)),
            y=numpy.random.normal(0, 1, int(1e6)),
            bins=(10, 10),
            range=((-5, 5), (-5, 5)),
        )
        after = aghast.to_numpy(aghast.from_numpy(before))
    ...
    def test_numpydd(self):
        before = numpy.histogramdd(
            (
                numpy.random.normal(0, 1, int(1e6)),
                numpy.random.normal(0, 1, int(1e6)),
                numpy.random.normal(0, 1, int(1e6)),
            ),
            bins=(5, 5, 5),
